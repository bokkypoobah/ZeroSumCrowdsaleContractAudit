...
var v1 = account3 + "0000000000000b3a73ce2ff2";
var v2 = account4 + "0000000000000b3a73ce2ff2";
// > new BigNumber("12345678901234").toString(16)
// "b3a73ce2ff2"
// -----------------------------------------------------------------------------
...
var transfer1Tx = token.transfer(account6, "1", {from: account3, gas: 100000});
var transfer2Tx = token.approve(account5,  "300", {from: account4, gas: 100000});
while (txpool.status.pending > 0) {
}
var transfer3Tx = token.transferFrom(account4, account7, "300", {from: account5, gas: 100000});
while (txpool.status.pending > 0) {
}
printBalances();
...
failIfTxStatusError(transfer1Tx, transferMessage + " - transfer 0.0001 tokens ac3 -> ac6. CHECK for movement");
...
var invalidTransfer1Tx = token.transfer(account7, "1", {from: account5, gas: 100000});
var invalidTransfer2Tx = token.approve(account8,  "300", {from: account6, gas: 100000});
while (txpool.status.pending > 0) {
}
var invalidTransfer3Tx = token.transferFrom(account6, account9, "300", {from: account8, gas: 100000});
while (txpool.status.pending > 0) {
}
printBalances();
...
failIfTxStatusError(invalidTransfer1Tx, invalidTransferMessage + " - invalid transfer 0.0001 tokens ac5 -> ac7. CHECK for NO movement");
